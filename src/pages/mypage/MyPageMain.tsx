...
const isOwner = useMemo(() => {
  ...
  const handleShortReviewClick = (reviewId: string) => {
    navigate(
      isOwner
        ? `/mypage/reviews/short/${reviewId}`
        : `/mypage/reviews/short/${viewedUser?.userId}`
    );
  };
        ...
        <SectionHeader>
          <SectionTitle
            onClick={() =>
              navigate(
                isOwner
                  ? "/mypage/movies/favorite"
                  : `/mypage/movies/favorite/${viewedUser.userId}`
              )
            }
          >
