    ...
    const fetchUserAndGenreData = useCallback(async () => {
        ...
        try {
            ...
            setUserProfile({
                ...
                profileImageUrl: apiData.profileImageUrl || 'https://via.placeholder.com/100/3498db/ffffff?text=User', // profileImageUrl이 없을 경우 기본값
                followerCount: apiData.followers,
                followingCount: apiData.following,
            });

            // 선택된 장르 ID 목록 설정
            const currentSelectedIds = apiData.userGenres.map(genre => genre.userGenreId);
            setSelectedGenreIds(currentSelectedIds);

        } catch (err) {
            console.error("사용자 프로필 및 장르 데이터를 불러오는 데 실패했습니다:", err);
    ...
    const handleGenreUpdate = useCallback(async (newGenreIds: number[]) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                setUpdateError(response.data.message || "장르 설정 저장에 실패했습니다.");
                alert('장르 설정 저장에 실패했습니다: ' + (response.data.message || "알 수 없는 오류"));
            }
        } catch (err) {
