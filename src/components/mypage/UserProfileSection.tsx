...
// ⭐ FollowStats를 Flex 컨테이너로 변경하여 자식 요소를 가로로 정렬
const FollowStats = styled.div`
  display: flex;
  justify-content: center;
  gap: 15px; /* 팔로워, 팔로잉 사이 간격 */
  color: #e0e0e0;
  font-size: 1.1em;
  margin: 5px 0 0;
  text-shadow: 1px 1px 3px rgba(0, 0, 0, 0.7);
...
// ⭐ 클릭 가능한 각 Follow 항목 스타일 추가
const FollowItem = styled.span`
  cursor: pointer;
  transition: color 0.2s ease-in-out;
  &:hover {
    color: #ff69b4; /* 호버 시 색상 변경 */
  }
`;
  ...
  return (
    <UserProfileSectionWrapper>
      ...
      <ProfileContent>
        ...
        {/* ⭐ FollowStats 내부를 클릭 가능한 FollowItem으로 분리 */}
        <FollowStats>
