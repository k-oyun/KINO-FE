...
// --- 폼 레이아웃 관련 스타일 ---
const FormContainer = styled.form` // as="form" 대신 직접 styled.form 사용
  display: flex;
...
// --- SettingForm 컴포넌트 ---
const SettingForm: React.FC<SettingFormProps> = ({ initialUserProfile }) => {
  const [profileImage, setProfileImage] = useState<string>(initialUserProfile.profileImageUrl);
  ...
  const hasChanges = profileImage !== initialUserProfile.profileImageUrl || nickname !== initialUserProfile.nickname;
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (canSave) {
      console.log("프로필 변경 요청:", { profileImage, nickname });
  ...
  return (
    <FormContainer onSubmit={handleSubmit}>
      ...
      <FormSection>
        ...
        <InputGroup>
          <CurrentNicknameDisplay>{initialUserProfile.nickname}</CurrentNicknameDisplay>
          <StyledInput
            type="text"
            id="nickname"
            value={nickname}
